...
Note that `data/` and `plots/` are both git-ignored, so everything the scripts write stays local. The four figures reproduced in this README are kept separately, as tracked copies under `docs/figures/`.
...
Both commands take relative paths, so they need to be run **from the repository root**, in this order:
